<template>
  <Popup v-model:show="isShow" closeable close-icon-position="top-right" close-icon="@/assets/images/close.png" @closed="emit('closed')"
    class="loginBg bg-transparent bg-center bg-no-repeat h-64% w-48vh max-w-90vw py-8vh px-6vh flex flex-col justify-between" >
    <!-- top -->
    <div>
      <!-- closeIco -->
      <div @click="emit('closed')" class="closeIco size-100px absolute top-3vh right-2.5vh"></div>
      <!-- 分段器 -->
      <el-segmented v-model="active" block size="large" :options="[{label:'邮箱登录',value:0},{label:'手机登录',value:1}]" 
          ...
          <el-form :model="formEmail" :rules="rules" ref="formEmailRef">
            ...
            <el-form-item prop="code" v-if="systemStore.systemData?.data.phone_check == OpenState.open">
              <el-input size="large" :placeholder="$t('验证码')" v-model="formEmail.code">
                ...
                <template #suffix>
                  <div class="send-code">
                    <SendCode :email="formEmail.username" scene="verify" :beforSend="beforSendEmail"></SendCode>
                  </div>
                </template>
              </el-input>
            </el-form-item>
        ...
        <Tab
          ...
          <el-form :model="formPhone" :rules="rules" ref="formPhoneRef">
            ...
            <el-form-item prop="phone">
              <el-input size="large" :placeholder="$t('输入手机号码')" v-model="formPhone.phone" inputmode="tel"
                        type="tel">
                <template #prefix>
            ...
            <el-form-item prop="code" v-if="systemStore.systemData?.data.phone_check == OpenState.open">
              <el-input size="large" :placeholder="$t('验证码')" v-model="formPhone.code">
                ...
                <template #suffix>
                  <div class="send-code">
                    <SendCode :qh="formPhone.qh" :phone="formPhone.phone" scene="verify" :beforSend="beforSendPhone">
                    </SendCode>
                  </div>
                </template>
              </el-input>
    ...
    <div>
      ...
      <div class="center flex-col my-30px space-y-30px"> 
        <el-button size="large" style="font-size: 14px;letter-spacing: 7px;text-indent: 7px;"  @click="login" :loading="loginLoading"
      ...
      <!-- link -->
      <div class="link-list">
        <div class="link text-white" @click="$router.push('/msgCenter?current=0'); isShow = false">{{ $t("客户服务") }}</div>
      </div>
    </div>
...
.el-segmented {
  ...
  --el-segmented-item-selected-bg-color: linear-gradient(to top right,#0083de, #3ca8f0);
