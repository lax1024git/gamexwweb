<template>
  <div class="card-box">
    ...
    <el-table :data="list" size="large" class="t-table" v-else>
      ...
      <el-table-column :label="$t('队伍总数')">
        <template #default="scope">
          {{ scope.row.child_user_num }}
        </template>
      </el-table-column>
...
import { v2_team_income_api } from "@/api/team";
import { ResCode } from "@/enum/ResultCode";
import { V2TeamIncomeItem } from "@/types/api/team";
...
const list: Ref<V2TeamIncomeItem[]> = ref([]);
const getList = async () => {
  loading.value = true;
  const res = await v2_team_income_api({
